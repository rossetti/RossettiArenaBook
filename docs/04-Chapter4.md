...
In this example, the performance measure of interest was the time to make the orders.  The value of TNOW at the end of the replication represents the time that all of the rings had been produced.  This was observed at the end of each replication by using an OUTPUT statistic, as per Figure \@ref(fig:LOTRConfig2CaptureOutput).  In many situations, we may want to compare performance on measures such as the average system time, the average number of jobs in the system, the resource utilization, etc.  In these situations, we need to capture the average of a tally variable or a time-persistent variable at the end of the replication.  As noted in Section \@ref(ch3s1), the functions, `TAVG(tally ID)` and `DAVG(dstat ID)` will collect the average of the within replication data. Thus, at the end of a replication, these functions hold the average performance of the corresponding tally or time-persistent variable for the current replication.  Thus, you can use `TAVG(tally ID)` and `DAVG(dstat ID)` in the expression field of the OUTPUT statistic module, as per Figure \@ref(fig:LOTRConfig2CaptureOutput), to capture the across replication statistics to a file for post processing using the Output Analyzer.  This will be useful in some of the exercises associated with this chapter.
